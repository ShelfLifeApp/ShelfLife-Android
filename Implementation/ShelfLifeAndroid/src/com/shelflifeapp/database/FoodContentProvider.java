...
public class FoodContentProvider extends ContentProvider {
	...
	public boolean onCreate() {
		this.database = new FoodDatabaseHelper(this.getContext(), 
				FoodDatabaseHelper.DATABASE_NAME, null, FoodDatabaseHelper.DATABASE_VERSION);
		return false;
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		if(sURIMatcher.match(uri) == FOOD_ID){
			...
			rowsDeleted = sqlDB.delete(FoodTable.DATABASE_TABLE_FOOD, FoodTable.FOOD_KEY_ID + "=" + id, null);
			if(rowsDeleted > 0){
				getContext().getContentResolver().notifyChange(uri, null);
			}
		}
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		if(sURIMatcher.match(uri) == FOOD_ID){
			String id = uri.getLastPathSegment();			
			rowsUpdated = sqlDB.update(FoodTable.DATABASE_TABLE_FOOD, values, FoodTable.FOOD_KEY_ID + "=" + id, null);
			if(rowsUpdated > 0){
				getContext().getContentResolver().notifyChange(uri, null);
			}
		}
